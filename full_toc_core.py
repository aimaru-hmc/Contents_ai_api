...
def process_pdf(pdf_path: Path, args: argparse.Namespace) -> Path | None:
    ...
    if args.stage == "layout":
        _parsed_path, layout_path, _parsed_text, _parsed_created = run_layout_stage(pdf_path, args, timestamp)
        return layout_path
...
def parse_pipeline_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR, help="산출물 기본 폴더")
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        log_info(f"Created output files: {len(created)}")
        for path in created:
        ...
        return {
            ...
            "failed_files": [
                {"pdf": str(pdf_path), "error": str(error), "type": type(error).__name__}
                for pdf_path, error in failures
            ],
        }
    finally:
...
API_FIELDS = {
    "input_dir", "input_file", "stage", "data_dir", "layout_dir", "full_json_dir", "log_dir",
    "layout_file", "parsed_dir", "force_parse", "pdf_column_mode", "pdf_column_split_x",
    "pdf_column_gap", "parse_mode", "max_heading_chars", "drop_author_lines",
    "drop_body_size_lines", "body_size_ratio_tolerance", "gemma_include_full_context",
    "title", "prompt", "max_depth", "openai_model", "openai_api_key", "openai_base_url",
    "openai_timeout", "openai_max_tokens", "openai_temperature", "openai_retries",
    "openai_retry_base_delay", "model", "ai_fallback_models", "ai_retries",
    "ai_retry_base_delay", "max_output_tokens", "gemma_chunk_mode", "gemma_max_context_tokens",
    "gemma_chunk_token_limit", "gemma_chunk_safety_tokens", "gemma_chunk_pages",
    "gemma_chunk_overlap_pages", "temperature", "base_url", "api_key", "timeout", "write_raw",
}
PATH_FIELDS = {"input_dir", "data_dir", "layout_dir", "full_json_dir", "log_dir", "layout_file", "parsed_dir"}
LIST_PATH_FIELDS = {"input_file"}
UPLOAD_PAYLOAD_FIELDS = {"filename", "file_base64", "files", "include_result"}
...
def args_from_payload(payload: dict[str, Any]) -> argparse.Namespace:
    if not isinstance(payload, dict):
        raise ValueError("JSON body must be an object.")
    args = parse_pipeline_args([])
    for raw_key, value in payload.items():
        key = normalize_api_key(str(raw_key))
        if key in UPLOAD_PAYLOAD_FIELDS:
            continue
        if key not in API_FIELDS:
            raise ValueError(f"Unsupported field: {raw_key}")
        if key in PATH_FIELDS:
            setattr(args, key, Path(value).expanduser() if value is not None else None)
        elif key in LIST_PATH_FIELDS:
            if value is None:
                setattr(args, key, None)
            elif isinstance(value, list):
                setattr(args, key, [Path(item).expanduser() for item in value])
            else:
...
def save_json_base64_uploads(payload: dict[str, Any], upload_dir: Path) -> list[Path]:
    uploaded: list[Path] = []
    if payload.get("file_base64"):
        raw = base64.b64decode(str(payload["file_base64"]), validate=True)
        uploaded.append(save_uploaded_pdf_bytes(upload_dir, str(payload.get("filename") or "uploaded.pdf"), raw))
    files = payload.get("files")
    if files is not None:
        if not isinstance(files, list):
            raise ValueError("files must be a list of {filename, content_base64} objects.")
        for item in files:
            if not isinstance(item, dict):
                raise ValueError("Each files item must be an object.")
            raw = base64.b64decode(str(item.get("content_base64") or item.get("file_base64") or ""), validate=True)
            uploaded.append(save_uploaded_pdf_bytes(upload_dir, str(item.get("filename") or "uploaded.pdf"), raw))
    return uploaded


def attach_uploaded_paths(args: argparse.Namespace, uploaded_paths: list[Path]) -> argparse.Namespace:
    if not uploaded_paths:
        return args
    existing = list(args.input_file or [])
    args.input_file = existing + uploaded_paths
    return args


def run_pipeline_locked(args: argparse.Namespace) -> dict[str, Any]:
    # The pipeline uses a process-global logger, so keep API runs serialized.
    with RUN_LOCK:
